...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Debug routes to help with login issues
  app.get('/api/debug/stores', async (req, res) => {
    try {
      const stores = await storage.getStores();
      res.json({ 
        count: stores.length,
        samples: stores.slice(0, 3).map(s => ({ 
          kodeGudang: s.kodeGudang, 
          namaGudang: s.namaGudang,
          hasPassword: !!s.storePassword 
        }))
      });
    } catch (error) {
  ...
  app.post('/api/debug/create-sample-data', async (req, res) => {
    try {
      ...
      const sampleStore = {
        kodeGudang: "TEST-01",
        namaGudang: "Test Store 1",
        jenisGudang: "Retail",
        storeUsername: "teststore",
      ...
      // Try to create store (ignore if exists)
      try {
        await storage.createStore(sampleStore);
      } catch (err) {
        // Store might already exist
      }

      // Try to create admin (ignore if exists)
      try {
        await storage.createStaff(sampleAdmin);
      } catch (err) {
